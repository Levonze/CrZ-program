...
static func check_three_fingers_toggle() -> void:
	var input := Engine.get_singleton("Input")
	var touch_count:int = input.get_connected_touch_count()
	if touch_count == 3:
		input.action_press("limbo_console_toggle")
